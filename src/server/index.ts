...
async function handleStart(root: string, port: number) {
...
async function handleUrl(root: string, port: number) {
  const existing = await getInstance(root);
  ...
  const child = spawn(
    process.execPath,
    [__filename, 'start', '--root', root, '--port', String(availablePort)],
    {
      cwd: process.cwd(),
      detached: true,
      stdio: 'ignore',
...
async function handleStop(root: string) {
  const existing = await getInstance(root);
